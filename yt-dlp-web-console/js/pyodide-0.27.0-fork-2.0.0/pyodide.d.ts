...
type PreRunFunc = (Module: Module) => void;
...
interface Module {
	...
	loadDynamicLibrary(lib: string, options?: {
		...
		global?: boolean;
		fs: LoadDynlibFS;
	}, localScope?: object | null, handle?: number): void;
...
interface API {
	...
	loadDynlibsFromPackage: (pkg: {
		file_name: string;
	}, dynlibPaths: string[]) => Promise<void>;
	install: (buffer: Uint8Array, filename: string, installDir: string, installer: string, source: string) => Promise<void>;
	recursiveDependencies: (names: string[], errorCallback: (err: string) => void) => Map<string, PackageLoadMetadata>;
	_Comlink: any;
	dsodir: string;
	sys: PyProxy;
	os: PyProxy;
	...
	finalizeBootstrap: (fromSnapshot?: SnapshotConfig, snapshotDeserializer?: (obj: any) => any) => PyodideInterface;
...
declare class PyodideAPI {
	...
	/** @hidden */
	static loadBinaryPackage: (name: string, buffer: ArrayBuffer | Uint8Array, format?: string) => Promise<void>;
	/** @hidden */
...
interface EmscriptenSettings {
	...
	readonly instantiateWasm?: false | ((imports: {
		[key: string]: any;
