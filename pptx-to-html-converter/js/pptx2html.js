$(document).ready(function() {
  ...
  if (window.Worker) {
    ...
    var runWorker = function(pptxArrayBuffer) {
    ...
    $("#downloadBtn").click(function(evt) {
      ...
      if (url) {
        fetch(url).then(res => res.arrayBuffer()).then(runWorker)
      }
      else {
...
Reveal.initialize({\
  ...
  dependencies: [\
    ...
    $("#result").delegate('img[src^="data:"]', "click", function(evt) {
      window.open(evt.target.getAttribute('src'), '_blank').focus()
    })

    // open PPTX URL if given in querystring
